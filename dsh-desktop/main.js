...
function ensureDesktopProfileInit() {
  try {
    ...
    if (fs.existsSync(source) && !fs.existsSync(link)) {
      ...
      try {
        // Windows 使用 junction，macOS/Linux 使用普通符号链接
...
async function startServer(unsafePortRetries = 4, overlays = []) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn(nodeBin, ['--use-system-ca', bin, '--profile', desktopProfile(), ...patchArgs, '--host', '127.0.0.1', '--port', String(webPort), '--no-open'], {
      cwd: userDataDir,
      env: childEnv(),
      windowsHide: true,
