...
module.exports = async function afterPack(context) {
  ...
  // 跳过非 Windows/macOS 平台
  if (electronPlatformName !== 'win32' && electronPlatformName !== 'darwin') return;
  ...
  console.log(`afterPack: bundled npm copied (deps: ${deps}) for ${electronPlatformName}`);
  ...
  // macOS 特定：设置 Node 可执行文件权限
  if (electronPlatformName === 'darwin') {
    const nodeBin = path.join(appOutDir, 'resources', 'node', 'node');
    if (fs.existsSync(nodeBin)) {
      fs.chmodSync(nodeBin, 0o755);
      console.log('afterPack: set node binary executable permission for macOS');
    }
  }
